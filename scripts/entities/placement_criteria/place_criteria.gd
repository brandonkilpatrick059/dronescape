...
@export var center_requires_groups : Array[String] = []
...
@export var center_excludes_groups : Array[String] = []
...
below : Array[Node2D], right : Array[Node2D], center : Array[Node2D]) -> bool:
		...
		has_all_required_groups = (has_all_required_groups &&
		check_required_groups(center,center_requires_groups))
		...
		no_excluded_groups = (no_excluded_groups && 
		check_excluded_groups(center,center_excludes_groups))
