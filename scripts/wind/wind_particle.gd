...
var queue_free_lock_timer := Timer.new()
...
func launch(force_vect : Vector2):
	...
	queue_free_lock_timer.one_shot = true
	add_child(queue_free_lock_timer)
	queue_free_lock_timer.start(1.0)
...
func update():
	...
	if(get_energy() > 0.4   &&
	!wind_streak_created && 
	randf_range(0.0,1.0) < 0.002): 
		var wind_streak_ref = wind_streak.instantiate()
		get_parent().add_child(wind_streak_ref)
		wind_streak_ref.global_position = global_position
		wind_streak_created = true
		wind_streak_ref.attach_particle(self)
	if(queue_free_lock_timer.is_stopped()):
		if(launched && global_position.distance_to(camera.global_position) > 400):
			queue_free()
		if(life_timer.is_stopped()):
			queue_free()
		if(launched && get_energy() < 0.1):
